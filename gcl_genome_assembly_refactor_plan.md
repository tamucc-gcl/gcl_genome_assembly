...
- [ ] **Organelle-target selection**: sheet already carries `meta.species` (+ optional `meta.taxid`; species falls back to global `params.mitohifi_species`). Derive **kingdom (plant vs animal)** from species/taxid via NCBI taxonomy — likely `taxonkit` against the taxdump the pipeline already downloads (name2taxid → lineage → kingdom), so plant SR samples attempt **mito + chloroplast**, animal SR samples **mito only**. (Add a per-run override param in case the lookup is ambiguous.)
- [ ] **Per-sample mito reference**: `FIND_MITO_REFERENCE` is currently called once globally with `params.mitohifi_species` (fine while all HiFi samples are one species). For multi-species HiFi runs, drive it per-sample from `meta.species`.
...
- [ ] **Parameter cleanup / systematization / unification pass.** As branches land, params proliferate (hifiasm ~40, plus SPAdes/jellyfish/genomescope/organelle) with inconsistent naming (`hifiasm_k` vs `spades_kmers`), scattered defaults (some in `nextflow.config`, some Elvis in-module, some in `main.nf`), and redundancy (multiple k-mer sizes, memory caps). Goal: one consistent scheme — grouped/namespaced params, defined once, documented in `nextflow.config`, still fully overridable per-run — without losing any current tunability. Best done once the input matrix is broad enough to see the full param surface (after short-read + organelles, likely alongside/after Phase 5). Requested by Jason 2026-07-06.
  - **Formalize the per-row-column ↔ global-param duality.** `buildMeta`'s `pick` already resolves `assembler`/`ploidy`/`n_hap`/`dedup`/`mito_tool`/`species`/`taxid` as *row column > global param > derived default*, so any field constant across samples can be a global param (`--ploidy 2`, `--n_hap 1`, …) with per-row override. But these global params (except `mitohifi_species`) are **undeclared** — they work via CLI (Nextflow injects the key) but aren't discoverable or config-settable until declared with null defaults in the params block. Also reconcile the naming: the `species` column falls back to `params.mitohifi_species` (not `params.species`) — pick one canonical name (likely `params.species`, with `mitohifi_species` as a deprecated alias). Deliver: declare + document all optional-column global params, consistent names. (Jason 2026-07-06.)
...
## 11. Documentation / README revamp (separate project)

A dedicated project (scope like the blobtools resurrection) to fully revamp the README and write end-user + developer documentation, once the input matrix and architecture stabilize. Not refactor cleanup — its own effort, best done after the pipeline's shape settles (short-read + organelles at least). Below is a **running list of what the docs must cover** — append to it as phases land.
...
- [ ] Input docs: the wide header-driven sample sheet — every column (required vs optional), with worked examples for each input combo (HiFi+Hi-C diploid, HiFi haploid, HiFi-only, short-read-only, +TellSeq later).
- [ ] Parameter reference: full list grouped by stage, defaults, meaning (pairs with the §10 param-unification pass).
- [ ] Outputs/results layout: directory tree, the QC report, per-stage artifacts, the assembly webshare.
- [ ] Architecture / developer docs: the meta-map model, subworkflow map, how to add an assembler/scaffolder/organelle tool, caching rules (what re-hashes and why).
- [ ] Per-tool notes + citations.

### Notes to fold into the docs (running — add as we build)
- **Parameter consolidation + column↔param duality** (§10): once params are unified, document the grouped scheme AND that optional strategy columns (`ploidy`, `n_hap`, `assembler`, `dedup`, `mito_tool`, `species`, `taxid`) can be set globally as params or per-row, with per-row winning.
- **`ploidy` (organism) vs `n_hap` (output haplotype count)**: distinct fields; the `n_hap` override; e.g. a collapsed diploid = `ploidy=2` + `n_hap=1` (hifiasm `--primary`), which is NOT a false haploid.
- **Assembler selection** (`meta.assembler`: hifiasm | spades) and how short-read routes through the single collapsed/`primary` path (SPAdes → n_hap=1 always).
- **Genome-size estimation** (jellyfish → GenomeScope2): `-p` = organism ploidy; where the estimate lands in the report; the summary.txt parse.
- **Organelle handling** (4b): MitoHiFi on HiFi (mito only, until a HiFi chloroplast tool exists), GetOrganelle on short-read; plant → mito + chloroplast via species/taxid → kingdom lookup.
- **Evidence-gating**: which steps run for which input types (Hi-C chain needs `meta.hic`; long-read finishing needs `meta.hifi`).
- (append more as phases land)
