...
- **2026-07-06 — First real (SLURM/conda) run: `kmer-jellyfish` conda fix + `ch_reads_all` per-sample de-barriering (delivered; pending real re-run).** Two issues from the first non-stub run. **(1) Conda:** the `genomescope` label used `bioconda::jellyfish`, which is a *different* package (conda's `jellyfish` is a string-matching library); the k-mer counter is `bioconda::kmer-jellyfish` (same `jellyfish` binary, so module commands unchanged). `bioconda::jellyfish` → not found → whole ESTIMATE_GENOME_SIZE env unsatisfiable (genomescope2 reported not-found only as collateral). Fix: config `conda = 'bioconda::genomescope2 bioconda::kmer-jellyfish'`. **(2) Per-sample barrier:** the 3× `join(remainder: true)` building `ch_reads_all` serialized ALL samples — `join` only emits an *unmatched* key once the right channel **closes**, and every sample is unmatched in ≥1 modality, so nothing emitted until `BAM_TO_FASTQ` + `TRIM_HIC` + `TRIM_SHORTREAD` all closed (blocking genome-size / contig-assembly / organelle for every sample; short-read SPAdes waited on unrelated HiFi BAMs + Hi-C trims). Fix: build per-modality **slot** channels (each sample gets one entry — real reads if it has that modality, else a `null` placeholder from `ch_input`, immediate) and use plain **1:1 joins**, which emit each key as soon as both sides have it (no wait for close). Each sample now advances at its own pace; only the genuine end-of-run aggregates (`QUAST_FINAL`/`COMPILE_FINAL_QC`/`SUMMARY_REPORT`) still wait for all. Output per sample unchanged → HiFi path caches. main.nf edit only (the `ch_reads_all` block); modules unaffected.
