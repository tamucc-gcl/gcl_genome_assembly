...
- **2026-07-06 — Phase 4a-ii: input gating + `ORGANELLE_ASSEMBLY` stub (delivered; pending 4-sample stub).** Makes the HiFi/Hi-C-assuming *input* steps conditional so short-read rows stop dying upstream, non-regressive on HiFi+Hi-C. (1) `BAM_TO_FASTQ` gated on `meta.hifi`, `TRIM_HIC` on `meta.hic` (upstream `.filter`, doesn't change task hashes for dip/hap). (2) `ch_reads_all` rebuilt from `ch_input` via two `remainder: true` left-joins against `BAM_TO_FASTQ.out`/`TRIM_HIC.out`, so a sample missing HiFi or Hi-C appears with null slots instead of being dropped by the old inner join (the 4a-i `ch_fastq_all` intermediate is gone). (3) New `workflows/organelle_assembly.nf` (`ORGANELLE_ASSEMBLY`): branches on `meta.hifi` — HiFi → MitoHiFi + circular map (identical inputs → cached), non-HiFi → **no-op stub** (4b: GetOrganelle); emits `mitogenome`/`annotation`/`stats`/`circular_map` (the exact surface main.nf consumed). main.nf: replaced the STEP-3b direct `MITOHIFI`/`MITO_CIRCULAR_MAP` calls with `ORGANELLE_ASSEMBLY(ch_reads_all→(meta,hifi,sr1,sr2), ref_fasta, ref_gb)`; repointed `FILTER_MITO_CONTIGS` + the 4 manifest/report refs to `ORGANELLE_ASSEMBLY.out.*`; `FIND_MITO_REFERENCE` stays global. **Expected on the 4-sample stub:** SPAdes finally runs for `plant`/`goby` (2 of 2) + genome-size 4 of 4; dip/hap identical; `plant`/`goby` then **drop at FILTER_MITO_CONTIGS** (no mitogenome — combine `by:0` is inner) — graceful, expected until 4a-iii gives them their own conditioning/finishing path. `MITOHIFI`/`MITO_CIRCULAR_MAP` main.nf includes now dead (owned by the subworkflow).
