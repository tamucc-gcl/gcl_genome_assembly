...
def inputTable(tablePath, required) {
    ...
    return Channel.of(sheet)
        .splitCsv(header: false, sep: ',', quote: '"', strip: true)
        .filter { cells -> cells.any { it?.toString()?.trim() } }
        .collect(flat: false)
        .ifEmpty { [] }
        .map { records ->
...
def parseSampleSheet(sampleSheetPath, hicReadsetsPath = null) {
    ...
    def libraryTable = hicReadsetsPath
        ...
        : Channel.value([sheet: null, rows: []])

    def validated = sampleTable.combine(libraryTable).map { table, libraries ->
    ...
    return [samples: validated.flatMap { it.samples }, status: validated.map { it.status }]
}
